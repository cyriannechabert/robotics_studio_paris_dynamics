...
# Pose A: (Your "step_1")
pose_A = {
    "FR_hip": 175.7, "FL_hip": 149.0, "BR_hip": 87.4, "BL_hip": 70.6,
    "FR_knee": 45.6, "FL_knee": 49.2, "BR_knee": 182.6, "BL_knee": 177.8
}
...
walk_cycle = [walk_angles, pose_B, walk_angles, pose_C]
...
def main_loop():
    ...
    move_to_pose(walk_angles, 1500)
    
    # This tracks which step is NEXT
    current_step_index = 0 
    
    print("\n--- Controls ---")
    print("  'w' = Take one step")
    print("  's' = Return to neutral")
    print("  'q' = QUIT")
    print("----------------")
    print("\nRobot is in neutral pose. Waiting for command...")
        ...
        while True:
            ...
            if msvcrt.kbhit():
                ...
                if key == 'q':
                    ...
                    move_to_pose(walk_angles, 1000)
                    current_step_index = 0  # Reset the walk cycle
                    print("Robot is in neutral pose. Waiting for command...")

                elif key == 'w':
                    # Get the pose to move to
                    pose = walk_cycle[current_step_index]
                    
                    print(f"Taking step {current_step_index + 1}...")
                    move_to_pose(pose, step_time_ms)
                    
                    # Update the index for the *next* press
                    current_step_index = (current_step_index + 1) % len(walk_cycle)
                    
                    print(f"Done. Waiting for command (w/s/q)... (Next step will be {current_step_index + 1})")
